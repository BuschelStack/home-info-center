<template>
  <section class="column" id="calllist">
    ...
    <div id="calllist-content">
      ...
      <template v-else>
        <div v-if="!sortedTags.length" class="no-data">Keine Daten vorhanden</div>
        <template v-else>
            ...
            <transition-group name="fade-list" tag="ul" class="call-list">
              ...
              >
                ...
                <span class="call-text">
                  {{ formatTime(call.date) }} | {{ call.name || call.caller || 'Unbekannt' }} |
                  {{ call.called || '–' }}
                  <span v-if="call.duration && call.duration !== '–'">
...
const TYPE_CLASSES = { 1: 'call-type-1', 2: 'call-type-2', 3: 'call-type-3', 11: 'call-type-11' }
const TYPE_ICONS = { 1: 'call-in.svg', 2: 'call-missed.svg', 3: 'call-out.svg', 11: 'voicemail.svg' }

function callTypeClass(type) {
  return TYPE_CLASSES[type] || 'call-type-other'
}

function getCallIcon(type) {
  const filename = TYPE_ICONS[type] || 'call-other.svg'
...
function formatDuration(durationStr) {
  const [hStr, mStr] = String(durationStr).split(':')
...
const { data, isLoading, error, dataUpdatedAt } = useApiQuery('calls', '/api/calls')

const callsData = computed(() => data.value?.calls || {})
const sortedTags = computed(() =>
  Object.keys(callsData.value).sort((a, b) => new Date(b) - new Date(a))
)
